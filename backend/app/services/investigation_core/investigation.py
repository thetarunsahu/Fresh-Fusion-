...
def investigate(db, sample):
    ...
    return {"inspection_id": sample.sample_id, "sample": sample_info(sample), "status": decision["status"],
            "evidence": evidence, "analysts": summarize_analysts(evidence, fusion),
            "critic": fusion["components"]["critic"], "decision": decision,
            "timeline": timeline, "timeline_note": "Newest 200 frames, 200 readings and 100 stored fusion results; rolling preview retention may remove older frames. Analysis events share their frame timestamp.",
