...
import { ArrowLeft, Eye, EyeOff, LockKeyhole, ShieldCheck, Sparkles, UserRound } from "lucide-react";
  ...
  return (
    <div className="authPage">
      <section className="authStoryPanel">
        <button className="backToHome" onClick={() => navigate("home")}><ArrowLeft size={16} /> Back to FreshFusion</button>
        ...
        <div className="authStoryCopy">
          ...
          <p>Authentication separates user access from device ingestion and gives human review a clear identity trail. The SIH prototype uses persisted users and signed JWT sessions.</p>
        ...
        <div className="authStoryFoot">Device authentication and secure ESP32 pairing remain a separate production-hardening step.</div>
      </section>
      ...
      <section className="authFormPanel">
        <div className="authFormWrap">
          <div className="authModeSwitch" role="tablist" aria-label="Authentication mode">
            ...
            <span className="ffJwtBadge"><ShieldCheck size={12} /> JWT ACTIVE</span>
          </div>
          ...
          <form onSubmit={submit} className="authForm">
            {mode === "register" && (
              <label><span>Full name</span><input value={fullName} onChange={(e) => setFullName(e.target.value)} minLength={2} maxLength={120} autoComplete="name" placeholder="Tarun Kumar Sahu" required /></label>
            )}
            <label><span>Email</span><input type="email" value={email} onChange={(e) => setEmail(e.target.value)} autoComplete="email" placeholder="you@example.com" required /></label>
            <label>
              <span>Password</span>
            ...
            {mode === "register" && (
              <label>
                ...
                <select value={role} onChange={(e) => setRole(e.target.value)}><option value="operator">Operator</option><option value="reviewer">Reviewer</option></select>
            ...
            <button className="authSubmit" type="submit" disabled={busy}>{busy ? "Please wait…" : mode === "login" ? "Sign in securely" : "Create account"}</button>
          </form>

          <div className="authSecurityNote">
            <ShieldCheck size={17} />
            <div><b>Prototype security boundary</b><p>User authentication is real and server-validated. ESP32/device authentication is intentionally tracked as a separate production feature so hardware integration is not broken by the dashboard login layer.</p></div>
          </div>
        </div>
      </section>
    </div>
